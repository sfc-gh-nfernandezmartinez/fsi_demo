...
Real-time transaction ingestion using Snowflake's Snowpipe Streaming API
"""
...
from snowflake.ingest.utils.logging_util import get_logger

console = Console()
logger = get_logger(__name__)

class SnowpipeStreamer:
    """Real-time transaction streaming using Snowpipe Streaming API"""
    ...
    def _get_snowflake_config(self) -> Dict[str, str]:
        ...
        try:
            ...
            raise FileNotFoundError("Snowflake CLI config not found")
    ...
    def _setup_ingest_manager(self):
        """Setup Snowpipe Streaming ingest manager"""
        try:
            # For now, we'll use regular Snowflake connector for direct INSERT
            # Snowpipe Streaming requires additional setup that's beyond basic demo
            console.print("📡 [yellow]Setting up Snowflake connection for streaming...[/yellow]")
            
            # Read private key
            with open(self.sf_config['private_key_path'], 'rb') as key_file:
            ...
            console.print("✅ [green]Snowflake connection established[/green]")
    ...
    def _send_to_snowflake(self, transaction: dict) -> bool:
        """Send transaction to Snowflake using streaming insert"""
        ...
        try:
            ...
            enhanced_transaction['ingestion_timestamp'] = datetime.now(timezone.utc).isoformat()
            ...
            logger.error(f"Failed to send transaction to Snowflake: {e}")
            return False
    
    def stream_transactions(self, duration_seconds: Optional[int] = None):
        """Stream transactions in real-time using Snowpipe"""
        if not self.sf_connection:
        ...
        console.print("🚀 [bold green]Starting Snowpipe Streaming[/bold green]")
        ...
        with Live(self._create_status_table(), refresh_per_second=2) as live:
            while self.running:
                ...
                # Send to Snowflake via streaming
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Snowpipe Streaming for FSI demo')
    ...
    streamer = SnowpipeStreamer(rate=args.rate, anomaly_rate=args.anomaly_rate)
