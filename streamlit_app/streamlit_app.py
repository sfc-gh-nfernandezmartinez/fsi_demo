...
def load_customer_summary():
    ...
    SELECT 
        ...
        COUNT(DISTINCT CASE WHEN TRANSACTION_DATE >= DATEADD('day', -30, CURRENT_DATE()) 
              THEN CUSTOMER_ID END) as ACTIVE_CUSTOMERS_30D,
        SUM(CASE WHEN TRANSACTION_DATE >= DATEADD('day', -30, CURRENT_DATE()) 
            THEN AMOUNT ELSE 0 END) as TOTAL_VOLUME_30D,
        COUNT(CASE WHEN TRANSACTION_DATE >= DATEADD('day', -30, CURRENT_DATE()) 
              THEN TRANSACTION_ID END) as TOTAL_TRANSACTIONS_30D
    FROM FSI_DEMO.ANALYTICS.CUSTOMER_360
    """
